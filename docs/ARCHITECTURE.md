...
flowchart TB
    subgraph Presentation["Presentation — src/viewer"]
        ...
        TERRAIN["Map, DTED, model, and cubemap readers"]
    end
...
| `src/viewer` | Composition, widgets, dialogs, camera policy, map/terrain formats, Plane, and HUD rendering | Application, domain, infrastructure only at the composition root | Business persistence or transport policy |
...
Plane terrain has a separate, presentation-owned path. It deliberately does not
enter the application runtime because it changes only visualization data:

```mermaid
flowchart LR
    POS["Validated aircraft position"] --> W["PlaneTerrainWorker\nlatest-only revision"]
    MAP["MapLandIndex\nimmutable land triangles"] --> W
    DTED["DtedTileSource + bounded cache"] --> W
    W --> PATCH["Immutable PlaneTerrainPatch\nmesh + land mask"]
    PATCH --> UI["PlaneSceneWidget\nUI/context thread"]
    UI --> GPU["PlaneTerrainGpuLayer"]
```

The worker reads and samples files, derives normals, classifies land/water, and
publishes an immutable candidate. The widget accepts only the current revision;
all OpenGL upload and destruction remains on the context thread.
...
Version 2 of the package also carries a 360-by-180 one-degree land-triangle
index. `MapLandIndex` exposes that immutable index to Earth and Plane, ensuring
both visualizations use the same vector coastline authority.
...
- The validated map mesh and its land index are immutable shared CPU values;
  Earth and Plane never rebuild a competing coastline data source.
...
- Terrain workers publish immutable, revision-tagged patches. Only the
  widget/context thread may install or destroy their GPU representation.
...
| New terrain elevation source | Add a bounded sampler behind the Plane terrain builder and preserve immutable patch publication | File IO or OpenGL calls in the UI-independent application core |
