...
assets, UDP input, mapping/session/model files, worker threads, and GPU upload
paths. It assumes the operating system, Qt runtime selected by the release, and
the user's account are not already compromised.
...
- A local file provider may supply malformed or adversarial mapping, `.lar`,
  `.gltf`, `.glb`, PNG, JPEG, map, mask, or DTED data.
...
CMake rejects Qt older than 6.10.3, while the release matrix installs 6.11.2.
...
smoke validates this component set; CI scans the generated SPDX document.
...
| Denial of service | Datagram flood, parser allocations, GPU uploads | Batches, parser budgets, sparse session index; diagnostic flood remains |
...
The following controls are release gates: strict conversion builds;
...
SBOM validation; and dependency scanning.
