...
| `Doxyfile` | Standalone warning-as-error Doxygen HTML/XML site configuration |
...
| Path | Purpose |
| --- | --- |
| `.github/workflows/windows.yml` | Current strict MSVC/Qt 6.11.2 build, deterministic test, and install-layout workflow |
...
| `cmake/LarPlaneAssets.cmake` | F-16/cubemap executable-adjacent staging hook |
| `cmake/StagePlaneAssets.cmake` | Locked, transactional model/cubemap staging script |
...
| `assets/map/world_boundaries.geojson` | Build-time vector land/coastline source; not installed directly |
...
| `src/viewer/map/map_mesh.h` | CPU geographic vertices, fill/border indices, and one-degree land cell ranges |
...
| `src/viewer/map/map_land_index.{h,cpp}` | Immutable indexed geographic land-triangle classification shared by Earth and Plane |
...
| `src/viewer/plane/plane_land_mask.{h,cpp}` | Adaptive vector-derived local land/sea raster builder |
...
| `tools/map_asset/map_mesh_compiler.{h,cpp}` | Projection, triangulation, border, and one-degree land-index construction |
...
| `tests/plane_view_tests.cpp` | GLB/cubemap, DT0/DT1/DT2 parsing/sampling, vector land masks, terrain workers, source switching, surface, attitude, camera, and controls |
...
| `docs/USER_GUIDE.md` | Operator workflow, recording/replay, views, terrain, and troubleshooting |
| `docs/VISUALIZATION.md` | Coordinate spaces, camera/rendering contracts, precision, and GPU lifecycle |
| `docs/TERRAIN_AND_ASSETS.md` | Map package, land index, DTED, terrain worker, glTF, cubemap, and staging contracts |
