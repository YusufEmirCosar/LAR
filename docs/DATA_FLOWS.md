...
sequenceDiagram
    ...
    M->>M: index land triangles into 360 x 180 degree cells
    M-->>W: MapMesh
...
The runtime never parses GeoJSON. The version-2 package carries both rendering
geometry and the one-degree land index, so Earth and Plane share one validated
coastline source. Revisions prevent a late asynchronous result from installing
after invalidation or a newer request.

## Plane terrain request and installation

```mermaid
sequenceDiagram
    participant UI as PlaneSceneWidget
    participant W as PlaneTerrainWorker
    participant D as DtedMosaicSampler
    participant L as PlaneLandMaskBuilder
    participant M as MapLandIndex
    participant B as PlaneTerrainPatchBuilder
    participant G as PlaneTerrainGpuLayer

    UI->>W: requestPatch(revision, source, center, extent, resolution, scale)
    W->>W: replace pending request; cancel superseded build
    W->>D: sample addressed DTED cells
    D-->>W: signed MSL elevations or missing samples
    W->>L: build(bounds, projection)
    L->>M: query candidate one-degree cells and triangles
    M-->>L: vector land candidates
    L-->>W: uniform classification or bounded R8 mask
    W->>B: build grid, normals, land class, water depth
    B-->>W: immutable PlaneTerrainPatch
    W-->>UI: patchReady(revision, patch)
    UI->>UI: accept only current source/revision
    UI->>G: install(patch) with current OpenGL context
```

Normal requests are aircraft-centered, use an odd 49–257 sample resolution,
and clamp half-extent to 20–60 km. The builder also applies defensive 1–100 km
and 17–257 limits. A patch can be reused while the aircraft remains within 35%
of its half-extent, at least 90% of the requested area is covered, and scene
scale still matches. Failed locations are not retried until movement exceeds
the larger of 5 km or 35% of the request extent.

A DT1/DT2 source change is transactional: the selected addressed cell must
parse before the worker is replaced. Failure therefore preserves the previous
source and visible patch. Missing terrain never creates a land-coloured flat
fallback; tactical overlays may remain visible without an opaque ground.
