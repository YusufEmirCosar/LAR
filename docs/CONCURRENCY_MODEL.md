...
`PlaneSceneWidget` creates `PlaneTerrainWorker` only while terrain work is
needed. Each source selection and patch request has a monotonically increasing
revision. New movement requests replace pending work; cancellation and source
replacement invalidate older results. A DT1/DT2 directory becomes active only
after its addressed cell validates, so a failed upload leaves the current
terrain source and visible patch intact. The worker may read DTED and rasterize
the immutable `MapLandIndex`, but it never touches a widget or OpenGL object.
The widget accepts the current patch and performs GPU replacement on its own
context thread.
...
- Cross-thread arguments are registered Qt metatypes and copied or shared as
  immutable values.
- Terrain requests are revisioned and latest-only; stale source or movement
  results cannot replace the current patch.
